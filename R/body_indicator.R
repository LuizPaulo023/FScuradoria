...
body_indicator <- function(indicator,ranking,access_type,name_en,name_pt,short_en,short_pt,
                           source_en,source_pt,description_en,description_pt,description_full_en,
                           description_full_pt,country,sector,node_en,node_pt,proj_owner,
                           token, url) {

  # Tibble para preenchimento \\\\\\\\\\\\\\\\\\\\\\\
  ...
  # Chamando as funções {get_tree} & {get_id} para obter os ID's

  tree_fs = FScuradoria::get_tree(master_node = base::strsplit(input$node_en[[1]], ", ")[[1]][1], token = token, url = url)
  id_nodes = FScuradoria::get_id(tree = tree_fs, node = stringr::str_trim(input$node_en[[1]]))

  # if(id_nodes$id[1] == 500) {
  #   return(500)
  # }

  # Body para preenchimento ====================================================

  id_nodes = tibble::tibble(node = id_nodes$node, id = id_nodes$id, node_pt = node_pt)
  ...
  send_fs = input %>%
    dplyr::mutate(body = body,
                  ...
                  c("posicao" = as.character(ranking),
                    ...
                    "name_one_tree_pt" = strsplit(input$node_pt[[1]], ",")[[1]][1]))) %>% dplyr::select(-body)
