...
get_id <- function(tree = as.list(),
  ...
  output <- tibble::tibble(node = tree$node, id = tree$id)
  ...
  if(node_lvls == 1) {
    ...
    for (lvl_n in 2:node_lvls) {
      n_children <- base::length(tree_children)

      #Opções de children
      for (lvl_c in 1:n_children) {
        check_node = ifelse(tree_children[[lvl_c]]$node == node_vct[[lvl_n]], T, F)
